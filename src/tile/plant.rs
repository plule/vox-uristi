...
pub fn collect_plant_voxels(
    ...
    let material = match tile.tile_type().material() {
        ...
        _ => {
            return collect_tree_voxels(tile, map, settings, raws);
        }
    };
    ...
    voxels_from_uniform_shape(shape, tile.coords(), material)
}
...
pub fn collect_tree_voxels(
    ...
    let mut voxels = voxels_from_uniform_shape(
        structure_shape(tile, &part, map),
        coords,
        structure_material,
    );
    let growth_materials = growth_materials(tile, &part, raws, settings.year_tick);
    if alive && !growth_materials.is_empty() {
        let growth = growth_shape(&part).map(|slice| {
            slice.map(|col| {
                col.map(|t| {
                    if t {
...
pub fn structure_shape(tile: &BlockTile, part: &PlantPart, map: &Map) -> Box3D<bool> {
